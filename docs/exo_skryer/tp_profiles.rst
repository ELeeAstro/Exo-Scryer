...
Exo_Skryer provides several analytical T-p profile functions in the `~exo_skryer.vert_Tp` module.
...
Required parameters: :math:`T_{\rm int}` [K], :math:`T_{\rm eq}` [K], :math:`\log_{10} \kappa_{\rm ir}` [cm\ :sup:`2` g\ :sup:`-1`], :math:`\log_{10} \gamma_v` (visible-to-IR opacity ratio), :math:`\log_{10}g` [cm s\ :sup:`-2`]
...
The `Madhu & Seager (2009) <https://ui.adsabs.harvard.edu/abs/2009ApJ...707...24M/abstract>`_ profile uses a piecewise polynomial parameterization with multiple pressure nodes, allowing for flexible T-p profiles with inversions.

Required parameters: :math:`a_1`, :math:`a_2` (polynomial coefficients), :math:`\log_{10} p_1`, :math:`\log_{10} p_2`, :math:`\log_{10} p_3` [bar] (pressure nodes), :math:`T_0` [K] (reference temperature)
...
.. plot::
   ...
   import numpy as np
   import matplotlib.pyplot as plt
   from exo_skryer.vert_Tp import MandS09
   from exo_skryer.data_constants import bar
   ...
   T_lev, T_lay = MandS09(p_lev, params)
   ...
   ax.set_xlabel('Temperature [K]', fontsize=16)
   ax.set_ylabel('pressure [bar]', fontsize=16)
   ax.set_title('Madhu & Seager (2009) T-p Profile', fontsize=14)
   ax.tick_params(labelsize=14)
   ax.invert_yaxis()
   plt.tight_layout()
...
.. code-block:: yaml
   ...
   params:
     - { name: a1, dist: uniform, low: -5.0, high: 5.0, transform: logit, init: 0.0 }
     - { name: a2, dist: uniform, low: -5.0, high: 5.0, transform: logit, init: 0.0 }
     - { name: log_10_p1, dist: uniform, low: -3, high: 2, transform: logit, init: -1 }
     - { name: log_10_p2, dist: uniform, low: -3, high: 2, transform: logit, init: 0 }
     - { name: log_10_p3, dist: uniform, low: -3, high: 2, transform: logit, init: 1 }
     - { name: T0, dist: uniform, low: 500.0, high: 3000.0, transform: logit, init: 1500.0 }
